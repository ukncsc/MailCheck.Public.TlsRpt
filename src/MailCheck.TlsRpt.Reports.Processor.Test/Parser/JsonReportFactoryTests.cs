...
namespace MailCheck.TlsRpt.Reports.Processor.Test.Parser
{
    ...
    public class JsonReportFactoryTests
    {
        ...
        [TestCase("no-policy-found", PolicyType.NoPolicyFound, "tlsa-invalid", ResultType.TlsaInvalid)]
        [TestCase("no-policy-found", PolicyType.NoPolicyFound, "validation-failure", ResultType.ValidationFailure)]
        public void CreateMapsKnownPropertiesBadReport(string rawPolicyType, PolicyType expectedPolicyType, string rawResultType, ResultType expectedResultType)
        {
            string rawReport = GetBadRawReport(rawPolicyType, rawResultType);
            byte[] normalisedAttachment = Encoding.ASCII.GetBytes(rawReport);
            JsonReport report = _attachmentParser.Parse(new AttachmentInfo(null, normalisedAttachment));
        ...
        [Test]
        public void SerializeJsonReportCorrectlyWithIncorrectMxHost()
        {
            var a = new JsonReport
            {
                OrganizationName = "org",
                DateRange = new DateRange() { EndDatetime = DateTime.UnixEpoch, StartDatetime = DateTime.UnixEpoch },
                ContactInfo = "con",
                ReportId = "rep",
                Policies = new List<PolicySummary>()
                {
                    new PolicySummary()
                    {
                        ...
                        {
                            PolicyDomain = "pol",
                            PolicyType = PolicyType.Tlsa,
                            PolicyString = new [] {"pol"},
                            MxHost = null
                        },
                        Summary = new Summary()
                    }
            ...
            using (StringWriter writer = new StringWriter())
            {
                ...
                jsonSerializer.Serialize(writer, a);
            }
        }

        [Test]
        public void SerializeJsonReportCorrectlyWithCorrectMxHost()
        {
            var a = new JsonReport
            {
                OrganizationName = "org",
        ...
        private string GetBadRawReport(string rawPolicyType = "sts", string rawResultType = "certificate-expired")
        {
            string json = @"
            {
                'organization-name': 'testOrganisationName',
